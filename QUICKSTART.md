...
Run the **AI SRE incident-response** cookbook end to end against a real, local NavFlow.

**What it shows:** the same incident-response agent investigates production faults two ways — the
provider-style way (each signal its own tool, fan out across them) and the NavFlow way (one `query`
...
## 1. Install and run NavFlow (the product)

NavFlow is the data plane the cookbook reads through — it ingests the demo system continuously and
serves the agent one correlated timeline over MCP.

```bash
uv tool install navflow            # or: pipx install navflow

navflow up                         # shell A: daemon + console → http://127.0.0.1:8787
navflow mcp                        # shell B: agent MCP endpoint → http://127.0.0.1:8788/mcp
```

Leave both running. (Docs: <https://www.navflow.ai/docs>.)

## 2. Bring up the cookbook's stack

Each cookbook ships its own system for NavFlow to ingest. For this one it's an api-server with
...
# one incident, all three variants (baseline fan-out → NavFlow query → NavFlow trigger):
...
On each run, `run.py` creates the cookbook's own **namespaced** NavFlow objects (`sre_*` sources, a
correlated view, and triggers) via the daemon's API — it never touches your existing sources —
injects the fault, forwards the platform's deploy/config into NavFlow, runs the agents, and resets.
...
> Model: default `claude-opus-4-8` (slow, ~1 min/agent). Override with `NAVFLOW_MODEL` — e.g.
> `NAVFLOW_MODEL=claude-haiku-4-5-20251001 python run.py latency_regression` finishes each agent in
...
```
$ python run.py latency_regression
NavFlow ready at http://127.0.0.1:8787: created 5 sources, view 'sre_service_timeline', triggers [...] (namespace 'sre_')
Incident: latency_regression  →  inject {'lever': 'inject_latency_ms', 'value': 800}   (expected trigger: sre_slow_responses)
injected the fault; forwarded the deploy + config into NavFlow.
...
→ navflow (one read): investigating (model=claude-opus-4-8)…
...
══════════════════════════════════════════════════════════════════
  ...
  navflow  (one read)    1 read                    turns=3   $0.15
  navflow  (triggered)   0 reads (woken by push)   turns=1   $0.12

  → NavFlow collapsed the read path 12× (12 → 1) and reached the same root cause.
...
The output leads with **what NavFlow did** — collapse the read path to one correlated read — not the
...
```bash
python run.py teardown                       # remove the cookbook's sre_* objects from NavFlow
cd cookbooks/01_sre_incident_response/platform && docker compose down   # stop the cookbook stack
# stop navflow up / navflow mcp with Ctrl-C in their shells
```
