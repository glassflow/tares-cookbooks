...
shared code context project. Idempotent: safe to run again.
...
def main() -> None:
    ...
    # The project's trigger fires on any commit younger than its window (5m). A repo that was just
    ...
    for full in repos:
        ...
        if age is not None and age < WINDOW_S:
            ...
            print(f"waiting {left}s: {full} has a commit younger than the trigger window; creating the project now would wake the agent on it")
            time.sleep(left)
    ...
    existing = tc.find_project()
    if existing:
        print(f"project already exists: {existing['id']} ({existing['status']})")
        return
    inst = tc.create_project(repos, ctx, branch=gh.default_branch(ctx),
                             layout=os.getenv("CONTEXT_LAYOUT", "per_repo"),
                             model=os.getenv("TARES_MODEL", ""))
    print(f"project created: {inst['id']} with {len(inst['objects'])} objects")
    print(f"open {tc.TARES_URL}/projects/{inst['id']} to watch it")
