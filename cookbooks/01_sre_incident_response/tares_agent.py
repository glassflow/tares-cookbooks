"""The Tares read path: the same SRE task as the baseline, but the fan-out tools are replaced by
...
`tares mcp`, discover its tools, and register only the one(s) we want. So even though the server
...
`async with`, so run.py opens it once and runs every Tares variant inside it.
"""
import os
...
from tares_client import VIEW, KEY

TARES_MCP_URL = os.getenv("TARES_MCP_URL", "http://127.0.0.1:8788/mcp")
...
SYSTEM_PROMPT = f"""You are an expert SRE incident response bot backed by Tares, a data plane that
...
REMEMBER_SYSTEM_PROMPT = f"""You are the same Tares-backed SRE agent. You have already diagnosed the
...
@asynccontextmanager
async def mcp_tools():
    """Open an MCP session to `tares mcp`, discover its tools, and hand back only the ones we
    register with the model — {'query': ..., 'remember': ...}. Everything else the server exposes
    stays out of the model's context."""
    async with streamablehttp_client(TARES_MCP_URL) as (read_s, write_s, _):
        async with ClientSession(read_s, write_s) as session:
            ...
            if missing:
                raise SystemExit(f"tares mcp is up but missing tools {sorted(missing)}. "
                                 f"Is the daemon seeded? (run.py calls tares_client.setup())")
