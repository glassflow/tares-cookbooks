...
through a **real, local Tares deployment**:
...
2. **Tares read path** — the *same* agent and the *same* prompt, but the fan-out tools are
   replaced by a single `query(view, key, window)` that Tares serves already correlated.

A third variant, **Tares trigger**, skips the investigation start entirely: Tares watches the
...
Grafana) with a real fault injected at runtime, and against the running product (`tares up` +
`tares mcp`). Same incident, same answer — the only thing that changes is how the agent reads.
...
| tares (one read) | 1 | 2 | 3,805 | $0.0194 | ✅ |
| tares (triggered, pushed) | 0 | 1 | 2,459 | $0.0129 | ✅ |
...
hard the incident is to disambiguate). Input tokens and cost come out **lower** for Tares too, and
...
by setting `include_payload=True` on the read, inflating Tares's tokens; Sonnet/Opus don't.)

**On the read-only measurement:** the baseline only ever reads, so to compare like-for-like the
Tares agent is measured read-only too. After the diagnosis, the Tares agent does one more thing
the baseline can't — it `remember()`s its conclusion back into Tares (`sre_memory`), so the next
...
tares mcp           # agent MCP endpoint → http://127.0.0.1:8788/mcp   (second shell)
```
...
`run.py` creates the cookbook's own Tares objects (see below), resets the system, subscribes a
webhook to the incident's trigger, injects the fault, forwards the platform's deploy + config into
Tares, waits for symptoms to register, then runs the baseline agent, the Tares agent, and (if
the trigger fires) the woken agent. It prints a **scoreboard** (the read-path collapse) and **what
Tares served** (the single correlated timeline) — not a wall of agent prose. It resets to healthy
...
### The cookbook is a guest on your Tares

It never touches your existing sources or relies on catalog import (which is one-shot and would
merge into your data). Instead `tares_client.setup()` creates its **own namespaced objects** via
...
`sre_` prefix is configurable via `TARES_COOKBOOK_NS`). They're obvious in the console, isolated
...
| `tares_agent.py` | The Tares read path. `mcp_tools()` opens an MCP client session to the running `tares mcp` and registers **only `query`** — even though the server advertises 13 tools, the model's context carries one schema. (It also wraps `remember` for the separate, unscored write-back.) |
...
| `tares_client.py` | The cookbook's Tares provisioning + I/O: `setup()`/`teardown()` for the namespaced `sre_*` objects, deploy/config push, `query_timeline()`, and subscribe/unsubscribe for the trigger webhook. |
| `run.py` | Stages one incident and runs all three variants (baseline, Tares, trigger). |
| `report.py` | Runs all four incidents through baseline + Tares and prints the consolidated metrics table. |
...
- The **system prompt** lives per-agent (in each `*_agent.py`), because the tools differ. The
  baseline's says "start with `get_service_health`, drill in with `query_metrics`…"; Tares's says
...
exactly the tools we register — the baseline its 5 in-process tools, Tares **one** (`query`). The
cookbook opens its own MCP client session to `tares mcp`, discovers its tools, and wraps only
`query` (`tares_agent.mcp_tools()`), so the server's other 12 tools never enter the model's
context. Each `query` is one MCP hop to the `tares up` daemon, which has been ingesting the
platform continuously via the cookbook's `sre_*` sources:

```
agent → query  (our MCP client → tares mcp, :8788)
              │  MCP call_tool
              ▼
      taresd  (tares up, :8787)  →  DuckDB  →  sre_service_timeline view
              ▲  continuous ingest (poll / push)
      Prometheus · container logs · deploys+config (webhook)
```

> Why not the Claude Agent SDK? Mounting `tares mcp` there loads *all 13* tool schemas into the
> model's context every turn (~3k tokens) — `allowed_tools` gates calling, not context. Owning the
> tools array (Tool Runner) is what lets Tares's read path be genuinely *one* tool.
...
rates and p99 scraped from Prometheus. The agent does the diagnosis; Tares only correlates. One
`query` returns the same facts the baseline gathered across 7–16 separate reads, already
time-ordered.

`sre_metrics` and `sre_logs` are polled by Tares's connectors (`prometheus`, `docker_logs`). The
...
symptoms and timing rather than read the answer off a config diff. That the Tares agent still
